...
\begin{flushleft}
    \begin{bfseries}
        \large{Version date: March 1, 2026}\\
...
\textsc{FEVtools} is a \textsc{FORTRAN} package of applications that includes a total of 23 tools. Some of these tools are devoted to format conversion involving the \verb-.fev-, \verb-.flg-, and \verb-.fcs- types. The remaining ones are intended to 8 different tasks, as listed below:
...
\begin{itemize}
 ...
 \item \verb-xyz-: coordinates generation;
...
\section{The tools}
The 23 FEV-tools are explicitely listed below:
\begin{itemize}
 ...
 \item \verb-flg2pov-: Tensor illustration generation in POVRAY from flag-graph;
 \item \verb-fcs2pov-: Tensor illustration generation in POVRAY from faces-info;
 \item \verb-fev2sum-: computes sum rules from the embedding-tensor;  
 \item \verb-flg2sum-: computes the sum rules from the flag-graph;
 \item \verb-fcs2sum-: computes the sum rules from the faces-info; 
 \item \verb-fcs2kid-: structure generation;
 \item \verb-fcs2smb-: polygonal symbol generation from faces-info;
 \item \verb-flg2smb-: polygonal symbol generation from flag-graph;
 \item \verb-fcs2xyz-: coordinates generation;
 \item \verb-flg2iso-: isomorphism checking from flag-graph; 
 \item \verb-fcs2iso-: isomorphism checking from faces-info; 
 \item \verb-flg2pri-: reduction to primitive cell from flag-graph;
 \item \verb-flg2pri-: reduction to primitive cell from faces-info;
 \item \verb-flg2sym-: computes the symmetry group from flag-graph;
 \item \verb-fcs2sym-: computes the symmetry group from faces-info.
\end{itemize}


\subsection{Compilation}
To compile the package, just edit the \verb-makefile- indicating your \textsc{FORTRAN} compiler and type \verb-make- to build all the 23 tools. Each tool is used by simply executing the corresponding binary in a terminal by typing ``\verb-./binary-''.

\subsection{A few words about input files}
Except for \verb-fcs2gen- and \verb-flg2iso-, all the tools deal with a single system and read an \verb-inp- file which contains a \verb-systemlabel- string in its first line. Such a string identifies the system being studied, with the structure's relevant input file being named as a \verb-systemlabel.fev-, \verb-systemlabel.flg-, \verb-systemlabel.b.flg-, \verb-systemlabel.fcs- or a \verb-systemlabel.b.fcs- file, depending on the tool in consideration. The \verb-flg2iso- tool deals with a pair of systems, whose \verb-systemlabel1- and \verb-systemlabel2- labels are also read from an \verb-inp- file (one system string per line). On the other hand, the \verb-fcs2gen- tool reads an integer (from an \verb-igen- file) which represents the previous generation of structures. We detail the use of each tool in the next sections.


\section{Conversion tools}
There are 7 tools for format conversion involving the \verb-.fev-, \verb-.flg-, and \verb-.b.fcs- formats. Upon simple execution, all of them read a \verb-inp- file containing only the \verb-systemlabel- string in the first line, and work on \verb-systemlabel.b.***- or \verb-systemlabel.***- files. Here is the list:

\begin{itemize}
 \item \verb-fev2flg-: embedding-tensor to flag-graph conversion - creates \verb-.flg- and \verb-.b.flg- files of a given structure out of its \verb-.fev- file;
 \item \verb-fev2fcs-: embedding-tensor to faces-info conversion - creates \verb-.fcs- and \verb-.b.fcs- files of a given structure out of its \verb-.fev- file;
 \item \verb-flg2fev-: flag-graph to embedding-tensor conversion - creates the \verb-.fev- file of a given structure out of its \verb-.flg- or \verb-.b.flg- file;
 \item \verb-flg2fcs-: flag-graph to faces-info conversion - creates \verb-.fcs- and \verb-.b.fcs- files of a given structure out of its \verb-.flg- or \verb-.b.flg- file;
 \item \verb-fcs2fev-: faces-info to embedding-tensor conversion - creates the \verb-.fev- file of a given structure out of its \verb-.fcs- or \verb-.b.fsc- file;
 \item \verb-fcs2flg-: faces-info to flag-graph conversion - creates \verb-.flg- and \verb-.b.flg- files of a given structure out of its \verb-.fcs- or \verb-.b.fsc- file;
 \item \verb-for2bin-: binary-formatted conversion - in case of existing formatted \verb-.flg- and/or \verb-.fcs- files, creates the corresponding unformatted \verb-.b.flg- and \verb-.b.fcs- binary files of a given structure, and vice-versa (creates formatted files out of unformatted data).
\end{itemize}

For the \verb-fev2flg- (\verb-fev2fcs-) tool, if there are two or more non-equivalent flag-graphs associated with the tensor, it will generate \verb-.flg- and \verb-.b.flg- files (\verb-.fcs- and \verb-.b.fcs- files) for all of them. In that case, the files for the first graph (structure) will be named as \verb-systemlabel.flg- and \verb-systemlabel.b.flg- (\verb-systemlabel.fcs- and \verb-systemlabel.b.fcs-), while the files for the second one will be named as \verb-systemlabel_2.flg- and \verb-systemlabel_2.b.flg- (\verb-systemlabel_2.fcs- and \verb-systemlabel_2.b.fcs-), and so on.

For \verb-flg2fev-, there is an important note. More than being used for \verb-.flg- to \verb-.fev- conversion, it can be used as a tool to build the \verb-.fev- file for a structure that you draw on a sheet of paper, for example. In your drawing, enumerate all the elements and create a ``fake'' \verb-.flg- with the list of flags. For the \verb-nface-, \verb-neigh_flag-, and \verb-flag_color- entries, write anything, they do not matter to build the embedding tensor. However, after generating the \verb-.fev- from your drawing from your face \verb-.flg-, you have to run \verb-fev2flg- to create the corret \verb-.flg- file.


\section{Creating an illustrative figure of the tensor}

Use the \verb-fev2pov- tool to read a system's \verb-systemlabel.fev- file and create a \verb-systemlabel.pov- file for an illustration of tensor in a perspective view, with 0/1 entries represented by red/blue cubes labelled with their values. It further illustrates the faces/edges/vertices axes in green, in a black background. The specific \verb-systemlabel.fev- file has its \verb-systemlabel- file read from the \verb-inp- file (containing only the \verb-systemlabel- string in the first line).

The alternative \verb-flg2pov- and \verb-fcS2pov- tools create the same \verb-systemlabel.pov- file out of the \verb-systemlabel.flg-/\verb-systemlabel.b.flg- and \verb-systemlabel.fcs-/\verb-systemlabel.b.fcs- files, respectively. However, the \verb-fev2pov- tool is the primary one for this purpose, since the \verb-flg2pov- and \verb-fcS2pov- tools internally convert the flag-graph and faces-info data into the embedding-tensor format, respectively, before generating the \verb-systemlabel.pov- file. 
...
The alternative \verb-flg2sum- and \verb-fcS2sum- tools create the same \verb-.sum- output from the \verb-.flg-/\verb-.b.flg- and \verb-.fcs-/\verb-.b.fcs- files, respectively. However, the \verb-fev2sum- tool is the primary one for this purpose, since \verb-flg2sum- and \verb-fcS2sum- first convert flag-graph and faces-info data into embedding-tensor format, respectively, before generating the \verb-systemlabel.sum- file. 
...
The \verb-fcs2sym- is a derived tool that does the same task as he \verb-flg2sym-, but from the \verb-.fcs- or \verb-.b.fcs- files of the structure. In practical terms, \verb-fcs2sym- internally convert the faces-info data from the two structures to flag-graphs and perform the same task as \verb-flg2sym- (which is the primary tool for symmetry identification).
...
The \verb-flg2smb- tool is a derived Utility that does the same task as he \verb-fcs2smb-, but from the \verb-.flg- or \verb-.b.flg- files of the structure. In practical terms, \verb-flg2smb- internally convert the flag-graph data to part of the faces-info data and perform the same task as \verb-fcs2smb- (which is the primary tool for symbol generation).

\section{Reduction to primitive cell}

Suppose you have a \verb-.flg- file that does not represent a primitive cell for a given structure. You can construct the \verb-.fev- file for the corresponding primitive cell with \verb-flg2pri-. You have to have the \verb-systemlabel.flg- file of your structure to make the reduction. In addition to the \verb-systemlabel.flg- file, you have to have the \verb-inp- file (containing only the \verb-systemlabel- string in the first line) in the same folder. The resulting file will be named as \verb-systemlabel-\verb-primitive.fev-.

The \verb-fcs2pri- tool is a derived Utility that does the same task as he \verb-flg2pri-, but from the \verb-.fcs- or \verb-.b.fcs- files of the structure. In practical terms, \verb-fcs2pri- internally convert the faces-info data to the corresponding flag-graph and perform the same task as \verb-flg2pri- (which is the primary tool for primitive cell reduction).

In addition to generating \verb-systemlabel-\verb-primitive.fev-, the \verb-flg2pri- and \verb-fcs2pri- tools print on the screen a message tellig if the input file corresponds to a primitive cell or the $N\times M$ cell size in the case of a conventional cell.

\section{Isomorphism checking}

Use the \verb-flg2iso- tool to verify if there is an isomorphism relation between two structures. You will need the \verb-.flg- or \verb-.b.flg- files for the two systems and the \verb-inp- file has to have the \verb-systemlabel- strings for both systems (one per line). Upon execution, the program will print a message saying
\begin{verbatim}
...
The \verb-fcs2iso- is a derived tool that does the same task as he \verb-flg2iso-, but from the \verb-.fcs- or \verb-.b.fcs- files of the structures. In practical terms, \verb-fcs2iso- internally convert the faces-info data from the two structures to flag-graphs and perform the same task as \verb-flg2iso- (which is the primary tool for isomorphism checking).
