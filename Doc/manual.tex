...
Each nonzero element of the embedding tensor defines an entity called a \emph{flag}. In other words, a flag is defined as a $(f,e,v)$ integer trio for which $\Phi_{f,e,v}=1$. When the involved edge is a bridge, each nonzero tensor element defines a pair of twin flags, which are basically two flags related by $(f',e',v')=(f,e,v)$. A flag can be represented graphically over the structure by an arrow that emerges from the corresponding vertex, along its corresponding edge, and has a mark (two small parallel segments) towards the corresponding cell. It also defines either a clockwise or a counterclockwise orientation around the face. In Fig.~\ref{fev-labelling} we illustrate the $(1,2,3) flag$, where the blue color of the flag symbol and the blue counterclockwise-oriented circle indicate its orientation. In Fig.~\ref{fev-labelling}d we illustrate the counterclockwise-oriented $(1,4,4)$ in red color.
...
The \textsc{FEVtools} package deals with three fundamenta file formats (\verb-.fev-, \verb-.flg-, and \verb-.fcs-), which are schematically illustrated in Fig.~\ref{file-formats}. Each carries different information related to a given structure. In the following, we detail each of them.


\begin{figure}[ht!]
\centering
\includegraphics[width=0.8\columnwidth]{./figs/file-formats.pdf}
\caption{Schematicillustration of the \protect\url{.fev}, \protect\url{.flg}, and \protect\url{.fcs} file formats.}
\label{file-formats}
\end{figure} 

\section{The .fev format}

In \textsc{FEVtools}, the embedding tensor is exchanged with the user through a \verb-.fev- file. This format is described below:
\begin{itemize}
...
The \verb-.fev- is an easy way for the user to input the studied structure. Other formats are more adequate to obtain further information, but they can be extracted from the \verb-.fev- file from specific tools.
...
\begin{itemize}
 \item A first line containing the four integers \verb-nflags-, \verb-nf-, \verb-ne-, and \verb-nv-, corresponding to the number of flags, faces, edges, and vertices of the structure, respectively;
...
The \verb-.flg- format is convenient to find symmetry maps that enable us to determine the symmetry group of a given structure or to determine if two structures are equivalent or not.

\section{The .fcs format}
The faces-info format contains structural information relative to each face of the system. In \textsc{FEVtools}, it can be exchanged with the user through a \verb-.fcs- file. This format is described below:
\begin{itemize}
...
\end{itemize}
A fluxogram detailing the tools and their purposes is illustrated in Fig.~\ref{fev-summary}. We detail the use of each of them in the next sections.
...
To compile it, just edit the \verb-makefile- indicating your \textsc{FORTRAN} compiler and type \verb-make- to build all the 15 tools.
...
\textbf{IMPORTANT NOTE} More than being used for \verb-.flg- to \verb-.fev- conversion, it can be used as a tool to build the \verb-.fev- file for a structure that you draw on a sheet of paper, for example. In your drawing, enumerate all the elements and create a ``fake'' \verb-.flg- with the list of flags. For the \verb-nface-, \verb-neigh_flag-, and \verb-flag_color- entries, write anything; they do not matter to build the embedding tensor. However, after generating the \verb-.fev- from the drawing of your face \verb-.flg-, you have to run \verb-fev2flg- to create the corret \verb-.flg- file.

\section{Isomorphism checking}

Use the \verb-flg2iso- executable to verify if there is an isomorphism relation between two structures. You will need the \verb-.flg- files for the two systems and the \verb-inp- file has to have the \verb-systemlabel- strings for both systems (one per line). Upon execution, the program will print a message saying
\begin{verbatim}
...
The \verb-for2bin- executable checks if the \verb-.flg- and \verb-.b.flg- files for the structure indicated in the \verb-inp- file exist. If only one of them exists, it creates the other. The same is done for the \verb-.fcs- and \verb-.b.fcs- files for the structure indicated in the \verb-inp-.
...
Use the \verb-fev2pov- executable to create a \verb-.pov- file for an illustration of the tensor in a perspective view, with 0/1 entries represented by red/blue cubes labelled with their values. It further illustrates the faces/edges/vertices axes in green, and a black background. In addition to the \verb-systemlabel.fev- file, you have to have the \verb-fev2pov- executable and a \verb-inp- file (containing only the \verb-systemlabel- string in the first line) in the same folder. Just execute it to create the \verb-systemlabel.pov- file.
...
\begin{itemize}
 ...
 \item A \verb-.mir- file with the structural elements unchanged under mirror reflections, and their corresponding maps (a single element can be part of more than one mirror map);
...
The \verb-fcs2kid- tool creates a generation of structures with $N+1$ faces from a parent generation of structures with $N$ faces by means of the add dimer algorithm. To run it, you have to have 
\begin{itemize}
...
During structure generation with the \verb-fcs2kid- tool, a \verb-.anc- file is also written for each generated system. It contains 1) the system label of the ancestor system, 2) the ancestor's face number in which the dimer is added, and 3) the indices for the two edges within the face in which we add the dimer atoms.

Suppose we want to generate a rough initial set of coordinates for a given target structure. You can do it with the \verb-fcs2xyz- tool. To use it, you will need the following files:
\begin{itemize}
...
The important quantities for the structure optimization are $k$, $k_a$, $q$, and $dt$. These can be determined by the user through the \verb-relax.par- file, which contains these quantities (one per line) in this sequence. The \verb-relax.par- file still has a fifth line containing an integer that tells if the program will draw a \verb-.svg- and a \verb-.pdf- file with an illustration of the atomic structure (0 for no, any other integer for yes). Below we show an example for this set of parameters in the \verb-relax.par- format.
\begin{verbatim}
1.0D0   # k
8.0D0   # ka
1.0D0   # q
...
It is worth remembering that the \verb-.fev- and \verb-.nxy- have very simple forms, as represented below.\\
\verb-graphene.fev-:
\begin{verbatim}
1    3    2
1
