...
REAL(KIND=8):: ka,theta,pi,q,v1(2),v2(2),v3(2),theta0,fa,dt,fd!,torque
! pi value
pi=DACOS(-1.0D0)
...
! torque=0
! DO i=1,nv
!   torque=torque+r(i,1)*f(i,2)-r(i,2)*f(i,1)
! END DO
! torque=torque
! IF(ABS(torque).gt.2*pi/180.0D0) torque=2*(pi/180.0D0)*(torque/ABS(torque))
! print*,torque/(pi/180.0D0)
...
! DO i=1,nv
!   r1=r(i,:)
!   r(i,1)=r1(1)*DCOS(torque)-r1(2)*DSIN(torque)
!   r(i,2)=r1(1)*DSIN(torque)+r1(2)*DCOS(torque)
! END DO
