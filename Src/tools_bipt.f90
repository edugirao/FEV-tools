...
IMPLICIT NONE
INTEGER:: i,c1,c2,f,nflags,flag_color(nflags),flag(nflags,3),ng
INTEGER:: ncluster,neigh_flag(nflags,3),neigh_flag2(nflags,3),flag_color2(nflags)
INTEGER:: cluster(nflags),cluster2(nflags),nf,nface(nf),neigh_flag3(nflags,3)
...
IMPLICIT NONE
INTEGER:: i,j,jj,l,nflags,neigh_flag(nflags,3),flag_color(nflags),ncluster,cluster(nflags)
...
IMPLICIT NONE
INTEGER:: i,j,m,nflags,ncluster,cluster(nflags),nf_test,nf,nface(nf),nface3(nf)
INTEGER:: neigh_flag(nflags,3),Adj(nflags,nflags),nface2(nf),f,flag(nflags,3)
LOGICAL:: ok(nflags),torus,done!,nface_test(nf)
! Look for an inter-cluster pair of v-neighbor flags
Adj=0
DO i=1,nflags
  Adj(i,neigh_flag(i,2:3))=1 ! we are excluding the face-neighs to have only the 2i cycles (breaking hexs and tetras)
END DO
ok=.false.
ncluster=0
cluster=0
